view: rpt_alg {
  derived_table: {
    sql: SELECT V.STAT_CURR,
          ...
          V.PLANT_COUNTRY,
          V.CLUSTER
      FROM eon-internal-bigquery.DATASET_RAW_ENVASES.rpt_ventas v
          LEFT JOIN eon-internal-bigquery.DATASET_RAW_ENVASES.CALENDAR c on v.CALDAY = c.CALDAY
          LEFT JOIN eon-internal-bigquery.DATASET_RAW_ENVASES.REGIONES r ON v.SALESORG = r.SALESORGANIZATION
      ...
      FROM eon-internal-bigquery.DATASET_RAW_ENVASES.rpt_ventas v
          ...
          LEFT JOIN (
              SELECT CAST(99999999 - CAST(GDATU AS NUMERIC) AS STRING) AS CALDAY,
                  ...
                  END UKURS,
                  c.date
              FROM eon-internal-bigquery.DATASET_RAW_ENVASES.ECC_PROD_TCURR
