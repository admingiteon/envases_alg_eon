...
FROM
  eon-internal-bigquery.DATASET_RAW_ENVASES.rpt_ventas v LEFT JOIN
  eon-internal-bigquery.DATASET_RAW_ENVASES.CALENDAR c on v.CALDAY=c.CALDAY LEFT JOIN
    eon-internal-bigquery.DATASET_RAW_ENVASES.REGIONES r ON v.SALESORG = r.SALESORGANIZATION
...
FROM
    eon-internal-bigquery.DATASET_RAW_ENVASES.rpt_ventas v LEFT JOIN
    eon-internal-bigquery.DATASET_RAW_ENVASES.CALENDAR c on v.CALDAY=c.CALDAY
WHERE
...
FROM
    eon-internal-bigquery.DATASET_RAW_ENVASES.rpt_ventas v LEFT JOIN
    eon-internal-bigquery.DATASET_RAW_ENVASES.CALENDAR c on v.CALDAY=c.CALDAY LEFT JOIN
    eon-internal-bigquery.DATASET_RAW_ENVASES.REGIONES r ON v.SALESORG = r.SALESORGANIZATION LEFT JOIN
    (
        SELECT
            CAST(99999999 - CAST(GDATU AS NUMERIC) AS STRING) AS CALDAY,
            TRIM(FCURR) FCURR, TRIM(TCURR) TCURR,
